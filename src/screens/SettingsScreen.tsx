  ...
  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
    <View
      style={{
        ...
        borderRightColor: C.textSecondary,
      }}
    />
    <View
      style={{
...
function Slider({
  ...
  const dec = () =>
    onChange(parseFloat(Math.max(min, value - step).toFixed(2)));
  const inc = () =>
    onChange(parseFloat(Math.min(max, value + step).toFixed(2)));

  return (
    <View style={sl.row}>
      <TouchableOpacity
        style={[sl.btn, { borderColor: color + '30' }]}
        onPress={dec}
        activeOpacity={0.7}
      >
        <View
...
export function SettingsScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    setModelUrlDraft(savedSettings.modelUrl);
    Animated.parallel([
  ...
  const saveModelUrl = () => {
    const error = validateGgufDownloadUrl(modelUrlDraft);
    setModelUrlError(error);
    if (!error) update('modelUrl', modelUrlDraft.trim());
  };
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            setModelUrlDraft(defaultSettings.modelUrl);
  ...
  const handleClearAllMemories = () => {
    Alert.alert(
      'Clear All Memories',
      'Are you sure you want to delete all saved memories? This will not affect your chat history or downloaded models.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear',
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.urlField}>
            <TextInput
              ...
              autoCorrect={false}
              multiline
              accessibilityLabel="Direct Hugging Face GGUF download URL"
            />
          </View>
          {modelUrlError ? (
            <Text style={styles.urlError} accessibilityRole="alert">
              {modelUrlError}
            </Text>
          ) : null}
          ...
          <TouchableOpacity
            ...
            accessibilityRole="button"
          >
            <Text style={styles.saveUrlButtonText}>Validate and save URL</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.card}>
          ...
          >
            {['Precise', 'Balanced', 'Creative'].map(preset => (
              <TouchableOpacity
                ...
                style={[
                  styles.segmentBtn,
                  currentStyle === preset && styles.segmentBtnActive,
                ]}
              ...
              >
                <Text
                  style={[
                    ...
                    currentStyle === preset && styles.segmentBtnTextActive,
                  ]}
                >
                  {preset}
                </Text>
          ...
          {currentStyle === 'Custom' && (
            ...
            >
              Currently using{' '}
              <Text style={{ color: C.accent, fontWeight: '700' }}>Custom</Text>{' '}
              advanced settings.
            </Text>
          )}
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.toggleRow}>
            <View style={{ flex: 1, gap: 3 }}>
              ...
              <Text style={styles.paramDesc}>
                Allow the assistant to reuse facts it explicitly saves from
                chat.
              </Text>
            </View>
            <Switch
          ...
          <TouchableOpacity
            style={styles.manageMemoryBtn}
            onPress={openMemoryModal}
            activeOpacity={0.7}
          >
            <Text style={styles.manageMemoryBtnText}>
              View & Manage Saved Memories
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.card}>
          <Text style={styles.privacyText}>
            Moonlight AI is designed as a private, local-first Android AI
            assistant. Chats, settings, downloaded models, and saved memories
            are stored on this device. Internet access is used for model
            downloads. Chat inference does not use external cloud AI APIs after
            a compatible model is installed. File attachments read selected text
        ...
        <View style={[styles.card, { gap: 0 }]}>
          ...
          <ParamItem
            label="Top K"
            desc="Token candidate pool size"
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            {memories.length > 0 && (
              ...
              >
                <TouchableOpacity
                  style={styles.clearMemoriesBtn}
                  onPress={handleClearAllMemories}
                >
                  <Text style={styles.clearMemoriesBtnText}>
                    Clear All Memories
                  </Text>
                </TouchableOpacity>
              </View>
...
const styles = StyleSheet.create({
  ...
  urlError: { color: C.red, fontSize: 12, lineHeight: 18 },
  saveUrlButton: {
    minHeight: 48,
    borderRadius: 12,
  ...
  saveUrlButtonText: { color: '#FFFFFF', fontSize: 13, fontWeight: '700' },
